...
static int lx_at(Lexer *L)
{
    if (L->pos < L->len)
    {
        return static_cast<unsigned char>(L->src[L->pos]);
    }
    return 0;
}
...
static int lx_peek(Lexer *L, int off)
{
    ...
    if (p < L->len)
    {
        return static_cast<unsigned char>(L->src[p]);
    }
    return 0;
}
...
static void lx_advance(Lexer *L)
{
    ...
    {
        L->line++;
        L->line_start = static_cast<int>(L->pos + 1);
    }
    if (L->pos < L->len)
    {
        L->pos++;
    }
    L->col = static_cast<int>(L->pos - L->line_start) + 1;
}
...
Token lexer_next(Lexer *L)
{
    ...
    if (c == '"')
    {
        ...
        char *buf = (char*)malloc(L->len - L->pos + 1);
        int i = 0;
    ...
    if (c == '=')
        ...
        single_op = T_LBRACKET;
    else if (c == ']')
        single_op = T_RBRACKET;
    else if (c == ',')
        single_op = T_COMMA;
    else if (c == ':')
        single_op = T_COLON;
    else if (c == ';')
        single_op = T_SEMICOLON;
    else if (c == '!')
        single_op = T_NOT;
    ...
    if (isalpha(c) || c == '_')
    {
        ...
        if (!strcmp(buf, "let"))
            ...
            tt = T_NOT;
        // THE BALLS EXTENSION
        else if (!strcmp(buf, "balls"))
            tt = T_LET;
        else if (!strcmp(buf, "big_balls"))
            tt = T_LET;
        else if (!strcmp(buf, "shared_balls"))
            tt = T_LET;
        else if (!strcmp(buf, "loop_your_balls"))
            tt = T_FOR;
        else if (!strcmp(buf, "spin_balls"))
            tt = T_WHILE;
        else if (!strcmp(buf, "if_balls"))
            tt = T_IF;
        else if (!strcmp(buf, "else_balls"))
            tt = T_ELSE;
        else if (!strcmp(buf, "switch_balls"))
            tt = T_SWITCH;
        else if (!strcmp(buf, "drop_balls"))
            tt = T_BREAK;
        else if (!strcmp(buf, "jiggle_balls"))
            tt = T_CONTINUE;
        else if (!strcmp(buf, "grab_balls"))
            tt = T_FUNC;
